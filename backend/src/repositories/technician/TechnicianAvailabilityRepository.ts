// repositories/TechnicianAvailabilityRepository.ts
import { Types } from "mongoose";
import { ITechnicianAvailabilityRepository } from "../../interfaces/repository/technician/ITechncianAvailabilityRepository";
import TechnicianAvailability, { ITechnicianAvailability, ITimeSlot } from "../../models/technician/TechnicianAvailabilitySchema";
...
export class TechnicianAvailabilityRepository implements ITechnicianAvailabilityRepository {
  ...
  async findByTechnicianAndDate(technicianId: Types.ObjectId, date: Date): Promise<ITechnicianAvailability | null> {
  ...
  async findByTechnicianAndDateRange(technicianId: Types.ObjectId, startDate: Date, endDate: Date): Promise<ITechnicianAvailability[]> {
  ...
  async upsertAvailability(availabilityData: Partial<ITechnicianAvailability>): Promise<ITechnicianAvailability> {
  ...
  async updateTimeSlot(technicianId: Types.ObjectId, date: Date, slotIndex: number, updates: Partial<ITimeSlot>): Promise<ITechnicianAvailability | null> {
    const setUpdate: any = {};
  ...
  async findActiveSlotRulesByTechnician(technicianId: Types.ObjectId): Promise<ISlotRule[]> {
    const now = new Date();
