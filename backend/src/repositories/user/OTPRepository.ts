...
export class OTPRepository extends BaseRepository<IOtpVerification> implements IOTPRepository {
  constructor() {
    super(OTPVerificationSchema as Model<IOtpVerification>);
  }

  async findLatest(phone?: string, email?: string, purpose?: string): Promise<IOtpVerification | null> {
    const query: any = {};
  ...
  async deleteMany(phone?: string, email?: string, purpose?: string): Promise<void> {
    const query: any = {};
    if (phone) query.phone = phone;
    if (email) query.email = email;
    if (purpose) query.purpose = purpose;
