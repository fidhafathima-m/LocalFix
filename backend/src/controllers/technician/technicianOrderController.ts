// controllers/technician/TechnicianOrderController.ts
...
class TechnicianOrderController {
  ...
  getTechnicianOrders = async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    try {
      ...
      if (!technicianId) {
        this.logger.warn("Get technician orders failed - authentication required", context);
        const errorResponse = ResponseHelper.unauthorized("Authentication required");
        res.status(errorResponse.statusCode).json(errorResponse);
        return;
      }

      const result = await this.orderService.getTechnicianOrders(technicianId, page, limit);
  ...
  getTechnicianOrderById = async (req: AuthRequest, res: Response): Promise<void> => {
    const technicianId = req.user?.id;
    ...
    try {
      ...
      if (!technicianId) {
        this.logger.warn("Get technician order failed - authentication required", context);
        const errorResponse = ResponseHelper.unauthorized("Authentication required");
        res.status(errorResponse.statusCode).json(errorResponse);
        return;
      }

      const result = await this.orderService.getTechnicianOrderById(technicianId, orderId);
  ...
  updateOrderStatus = async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    try {
      ...
      if (!technicianId) {
        this.logger.warn("Update order status failed - authentication required", context);
        const errorResponse = ResponseHelper.unauthorized("Authentication required");
        res.status(errorResponse.statusCode).json(errorResponse);
        return;
      }
      ...
      if (!status) {
        this.logger.warn("Update order status failed - status required", context);
        const badRequestResponse = ResponseHelper.badRequest("Status is required");
  ...
  getTechnicianOrderStats = async (req: AuthRequest, res: Response): Promise<void> => {
    const technicianId = req.user?.id;
    ...
    try {
      ...
      if (!technicianId) {
        this.logger.warn("Get technician order stats failed - authentication required", context);
        const errorResponse = ResponseHelper.unauthorized("Authentication required");
        res.status(errorResponse.statusCode).json(errorResponse);
        return;
      }

      const result = await this.orderService.getTechnicianOrderStats(technicianId);

      this.logger.info("Technician order stats retrieved successfully", context);
