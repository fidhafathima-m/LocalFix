...
export class OrderController {
  ...
  getUserOrders = async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    try {
      ...
      if (!userId) {
        this.logger.warn('Get user orders failed - authentication required', context);
  ...
  cancelOrder = async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    try {
      ...
      if (!userId) {
        this.logger.warn('Cancel order failed - authentication required', context);
        const errorResponse = ResponseHelper.unauthorized("Authentication required");
        res.status(errorResponse.statusCode).json(errorResponse);
        return;
      }
      ...
      const result = await this.orderService.cancelOrder(userId, orderId, reason);
...
createOrderFromBooking = async (req: AuthRequest, res: Response): Promise<void> => {
  const userId = req.user?.id;
  ...
  try {
    ...
    if (!userId) {
      this.logger.warn('Create order failed - authentication required', context);
      const errorResponse = ResponseHelper.unauthorized("Authentication required");
      res.status(errorResponse.statusCode).json(errorResponse);
      return;
    }
    ...
    if (!bookingId || !paymentData) {
      this.logger.warn('Create order failed - missing required fields', context);
      const badRequestResponse = ResponseHelper.badRequest("Booking ID and payment data are required");
      res.status(badRequestResponse.statusCode).json(badRequestResponse);
      return;
    }
