...
export class PaymentController {
  ...
  createPaymentOrder = async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    try {
      ...
      if (!userId) {
        this.logger.warn('Create payment order failed - authentication required', context);
      ...
      if (!bookingId || !amount || !type) {
        this.logger.warn('Create payment order failed - missing required fields', context);
        const errorResponse = ResponseHelper.badRequest("Booking ID, amount and type are required");
        res.status(errorResponse.statusCode).json(errorResponse);
        return;
      }
      ...
      const errorResponse = ResponseHelper.error("Failed to create payment order");
      res.status(errorResponse.statusCode).json(errorResponse);
    }
  };
  ...
  verifyPayment = async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    try {
      ...
      if (!userId) {
        this.logger.warn('Verify payment failed - authentication required', context);
        const errorResponse = ResponseHelper.unauthorized("Authentication required");
        res.status(errorResponse.statusCode).json(errorResponse);
        return;
      }
      ...
      if (!razorpay_payment_id || !razorpay_order_id || !razorpay_signature) {
        this.logger.warn('Verify payment failed - missing payment data', context);
        const errorResponse = ResponseHelper.badRequest("Payment verification data is required");
        res.status(errorResponse.statusCode).json(errorResponse);
        return;
      }
