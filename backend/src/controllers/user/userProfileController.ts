...
export class UserProfileController {
  ...
  getUserProfile = async (req: Request, res: Response) => {
    try {
      ...
      return res.status(500).json(ResponseHelper.error("Failed to fetch user profile"));
    }
  };
  ...
  updateUserProfile = async (req: Request, res: Response) => {
    try {
      ...
      const result = await this.userProfileService.updateUserProfile(userId, updateData);
      ...
      return res.status(500).json(ResponseHelper.error("Failed to update user profile"));
    }
  };
  ...
  uploadProfilePicture = async (req: Request, res: Response) => {
    try {
      ...
      const result = await this.userProfileService.uploadProfilePicture(userId, req.file);
      ...
      return res.status(500).json(ResponseHelper.error("Failed to upload profile picture"));
    }
  };
}