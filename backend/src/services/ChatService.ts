...
Your role: Help customers with appliance repairs, pricing, booking, and technical support. Be friendly and specific.`;
  ...
  private getEnhancedFallback(
    ...
    const lastUserMessage =
      history.length > 0 ? history[history.length - 1]?.text.toLowerCase() : '';
    ...
    if (
      ...
      return {
        message:
          'Yes, we repair blenders! Our technicians can fix motor issues, blade problems, leakage, and electrical faults. Service starts from ₹299. Would you like to book a blender repair or need more details?',
        isRealAI: false,
      };
    }

    if (
      lowerMessage.includes('washing machine') ||
      lastUserMessage.includes('washing machine')
    ) {
      if (lowerMessage.includes('noise') || lowerMessage.includes('loud')) {
        return {
          message:
            'For washing machine noise issues, it could be due to unbalanced load, worn-out bearings, or drum problems. Our technicians can diagnose and fix this starting from ₹499. Would you like to schedule a repair visit?',
          isRealAI: false,
        };
      }
      return {
        message:
          'We provide comprehensive washing machine repair services including motor issues, drainage problems, spin cycle faults, and leakage. Service starts from ₹399. What specific issue are you facing?',
        isRealAI: false,
      };
    }
    ...
    if (
      ...
      return {
        message:
          'Yes, we repair refrigerators! Common issues we fix: cooling problems, compressor issues, thermostat faults, leakage, and ice maker problems. Service starts from ₹599. Could you describe the issue?',
        isRealAI: false,
      };
    }
    ...
    if (
      ...
      return {
        message:
          'We specialize in AC repair services including gas refilling, compressor issues, cooling problems, and maintenance. Service starts from ₹699. Are you facing a specific AC issue?',
        isRealAI: false,
      };
    }
    ...
    if (lowerMessage.includes('tv') || lowerMessage.includes('television')) {
      return {
        message:
          'We repair all types of TVs - LED, LCD, Smart TVs. Common fixes: screen issues, sound problems, power supply faults, and connectivity issues. Service starts from ₹799. What TV problem are you experiencing?',
        isRealAI: false,
      };
    }
    ...
    if (lowerMessage.includes('thank') || lowerMessage.includes('thanks')) {
      return {
        message:
          "You're welcome! Is there anything else I can help you with regarding appliance repairs or service booking?",
        isRealAI: false,
      };
    }
