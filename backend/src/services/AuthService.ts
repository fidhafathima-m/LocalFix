...
export class AuthService {
  ...
  async verifyOtp(otpData: OTPVerificationData): Promise<AuthResponse> {
    try {
      ...
      const record = await this.otpRepository.findLatest(phone, email, "signup");
  ...
  async login(credentials: LoginCredentials): Promise<AuthResponse> {
    try {
      ...
      const user = await this.userRepository.findByIdentifier(identifier, role);
      ...
      if (user.isDeleted) {
        return { success: false, message: "Your account has been deleted. Please contact support." };
      }

      if (user.status === "Blocked") {
        return { success: false, message: "Your account is blocked by admin. Please contact support." };
      }
  ...
  async forgotPassword(phone?: string, email?: string, userType?: string): Promise<AuthResponse> {
    try {
      if (!phone && !email) {
  ...
  async resetPassword(resetData: ResetPasswordData): Promise<AuthResponse> {
    try {
      ...
      await this.userRepository.updatePassword(identifier, passwordHash, userType);
...
async resendOTP(phone?: string, email?: string, purpose?: string, userType?: string): Promise<AuthResponse> {
  try {
...
async googleAuth(socialData: SocialAuthData): Promise<AuthResponse> {
  try {
    ...
    let socialAccount = await this.socialAccountRepository.findByProviderId(googleId);
    let user;

    if (socialAccount) {
      user = await this.userRepository.findById(socialAccount.userId.toString());
    } else {
