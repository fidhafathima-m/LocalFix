...
class UserServiceService {
    ...
    async getServiceById(serviceId) {
        ...
        try {
            ...
            if (service.status !== 'active') {
                ...
                throw new Error(constants_1.SERVICE_MESSAGES.SERVICE_NOT_AVAILABLE);
            }
    ...
    async getAllServices(page = 1, limit = 10, search, sortBy = 'name', sortOrder = 'asc') {
        const context = {
        ...
        try {
            ...
            if (search) {
                ...
                services = await this._serviceRepository.search(search, limit, sortOptions);
                ...
                services = await this._serviceRepository.findAll(filter, skip, limit, sortOptions);
    ...
    async getServicesByCategoryId(categoryId, page = 1, limit = 10, search) {
        const context = {
        ...
        try {
            this._logger.info('Fetching active services by category ID for user', context);
            ...
            if (search) {
                ...
                services = await this._serviceRepository.searchByCategory(categoryId, search, limit);
                // Filter only active services from search results
                services = services.filter(service => service.status === 'active');
                total = services.length;
                ...
                this._logger.debug('Fetching all active services in category with pagination', {
                    ...context,
                    skip,
                    limit,
                });
                services = await this._serviceRepository.findAll(baseFilter, skip, limit);
            ...
            const result = (0, serviceMapper_1.toServiceListResponseDto)(services, total, page, limit);
            this._logger.info('Active services by category retrieved successfully for user', {
                ...context,
                totalServices: total,
                returnedServices: result.services.length,
            });
    ...
    async searchServices(query, limit = 10) {
        ...
        try {
            ...
            return activeServices.map(service => (0, serviceMapper_1.toServiceResponseDto)(service));
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
