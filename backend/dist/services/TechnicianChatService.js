...
exports.TechnicianChatService = void 0;
...
class TechnicianChatService {
    ...
    async sendMessage(userMessage, conversationHistory = [], context, technicianId) {
        ...
        if (!this.shouldUseRealAI()) {
            ...
            return this.getTechnicianFallbackWithUsage(userMessage, conversationHistory);
        }
        ...
        const messages = this.buildTechnicianMessages(userMessage, conversationHistory, context);
        for (const model of availableModels) {
            ...
            try {
                ...
                if (error.response?.status === 429) {
                    ...
                    if (providerMessage.includes('rate-limited upstream') ||
                        providerMessage.includes('rate limit')) {
                        this._logger.warn('⏰ Model-specific rate limit, trying next model...');
                ...
                if (error.response?.status === 400) {
                    const providerMessage = error.response?.data?.error?.message || '';
                    if (providerMessage.includes('system message') ||
                        providerMessage.includes('instruction')) {
                        this._logger.warn('🔄 Model doesnt support system messages, trying without...');
                        // Try without system message for this model
                        const messagesWithoutSystem = this.buildMessagesWithoutSystem(userMessage, conversationHistory);
                        try {
                            const retryResponse = await axios_1.default.post(this._apiUrl, {
                                model: model,
                            ...
                            if (retryResponse.data.choices &&
                                ...
                                this._logger.info('✅ TECHNICIAN AI SUCCESS (without system message)!', { model });
                                this._rateLimitExceeded = false;
                                return {
                                    message: aiResponse,
                            ...
                            this._logger.warn(`❌ Retry without system also failed for ${model}`);
                            continue;
                        }
                    }
                }
    ...
    buildMessagesWithoutSystem(userMessage, conversationHistory, context) {
        const messages = [];
    ...
    buildTechnicianMessages(userMessage, conversationHistory, context) {
        const messages = [];
    ...
    getTechnicianFallbackWithUsage(userMessage, history = []) {
    ...
    getTechnicianSystemPrompt(context) {
...
Always provide specific, actionable advice. Focus on practical solutions that can be implemented on-site.`;
    ...
    getTechnicianFallback(userMessage, history = []) {
