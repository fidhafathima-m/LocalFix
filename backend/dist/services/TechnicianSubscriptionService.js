...
exports.TechnicianSubscriptionService = void 0;
class TechnicianSubscriptionService {
    constructor(subscriptionService, subscriptionRepository, subscriptionWalletService, subscriptionPaymentService, logger) {
    ...
    async createRazorpayOrder(subscriptionId, userId) {
        const context = {
        ...
        try {
            ...
            const subscription = await this._subscriptionService.getSubscriptionById(subscriptionId);
            if (subscription.status !== 'active') {
                throw new Error('Subscription is not active');
            }
            // Check if user already has an active subscription
            const activeSubscription = await this._subscriptionRepository.findActiveSubscription(userId);
    ...
    async processWalletPayment(subscriptionId, userId) {
        const context = {
        ...
        try {
            ...
            const debitResult = await this._walletService.debit(userId, subscription.price, 'subscription_purchase', `Purchase of ${subscription.name} subscription`);
    ...
    async verifyAndActivateSubscription(razorpayPaymentId, razorpayOrderId, razorpaySignature, subscriptionId, userId) {
        const context = {
        ...
        try {
            this._logger.info('Verifying payment and activating subscription', context);
            // Check if subscription already exists for this payment
            const existingSubscription = await this._subscriptionRepository.findByTransactionId(razorpayPaymentId);
            ...
            const verificationResult = await this._paymentService.verifyPayment(razorpayPaymentId, razorpayOrderId, razorpaySignature);
            ...
            const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
            this._logger.error('Payment verification and subscription activation failed', {
                ...context,
                error: errorMessage,
            });
            throw error;
    ...
    async checkSubscriptionEligibility(technicianId, subscriptionId) {
    ...
    async getCurrentSubscription(technicianId) {
        ...
        try {
            ...
            const currentSubscription = await this._subscriptionRepository.findActiveSubscription(technicianId);
    ...
    async getSubscriptionHistory(technicianId) {
        const context = {
        ...
        try {
            ...
            const subscriptionHistory = await this._subscriptionRepository.findByTechnicianId(technicianId);
    ...
    async getSubscriptionPurchaseById(purchaseId, technicianId) {
        const context = {
        ...
        try {
            ...
            const subscriptionPurchase = await this._subscriptionRepository.findById(purchaseId);
            // Verify that the purchase belongs to the technician
            if (subscriptionPurchase &&
                subscriptionPurchase.technicianId.toString() !== technicianId) {
                throw new Error('Access denied: This subscription does not belong to you');
            }
            this._logger.info('Subscription purchase retrieved successfully', context);
