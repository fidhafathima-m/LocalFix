...
class ReviewService {
    constructor(reviewRepository, logger, socketService) {
        this._logger = logger;
    ...
    async createReview(userId, reviewData) {
        ...
        try {
            ...
            if (reviewData.rating < 1 || reviewData.rating > 5) {
                ...
                return responseHelper_1.ResponseHelper.badRequest('Rating must be between 1 and 5');
            }
            // Validate comment
            if (!reviewData.comment || reviewData.comment.trim().length === 0) {
                this._logger.warn('Empty comment provided', context);
                return responseHelper_1.ResponseHelper.badRequest('Comment is required');
            }
            // Check if user can review this order
            const canReview = await this._reviewRepository.canUserReviewOrder(userId, reviewData.orderId);
            if (!canReview) {
                this._logger.warn('User cannot review this order', context);
                return responseHelper_1.ResponseHelper.badRequest("Cannot review this order. Order may not be completed, doesn't exist, or already has a review.");
            }
            ...
            const reviewModel = (0, reviewMapper_1.toReviewCreateModel)({
                ...reviewData,
                userId,
            ...
            const reviewDto = (0, reviewMapper_1.toReviewDto)(newReview);
            return responseHelper_1.ResponseHelper.success('Review submitted successfully', reviewDto);
        }
        catch (error) {
            ...
            return responseHelper_1.ResponseHelper.error('Failed to submit review');
        }
    }
    async updateReview(userId, reviewId, reviewData) {
        ...
        try {
            ...
            if (!existingReview) {
                ...
                return responseHelper_1.ResponseHelper.notFound('Review not found');
            }
            ...
            if (existingReview.userId && typeof existingReview.userId === 'object') {
                const userObj = existingReview.userId;
            ...
            if (reviewUserId !== userId) {
                ...
                return responseHelper_1.ResponseHelper.forbidden('You can only update your own reviews');
            }
            ...
            const updateModel = (0, reviewMapper_1.toReviewUpdateModel)(reviewData);
            const updatedReview = await this._reviewRepository.update(reviewId, updateModel);
            if (!updatedReview) {
                this._logger.error('Failed to update review in repository', context);
                return responseHelper_1.ResponseHelper.error('Failed to update review');
            }
            ...
            const reviewDto = (0, reviewMapper_1.toReviewDto)(updatedReview);
            return responseHelper_1.ResponseHelper.success('Review updated successfully', reviewDto);
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
    ...
    async deleteReview(userId, reviewId) {
        ...
        try {
            ...
            if (reviewUserId !== userId) {
                ...
                return responseHelper_1.ResponseHelper.forbidden('You can only delete your own reviews');
            }
            ...
            if (!deleted) {
                ...
                return responseHelper_1.ResponseHelper.error('Failed to delete review');
            }
            ...
            const reviewDto = (0, reviewMapper_1.toReviewDto)(existingReview);
            return responseHelper_1.ResponseHelper.success('Review deleted successfully', reviewDto);
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
    ...
    async getReviewById(reviewId) {
        ...
        try {
            ...
            return responseHelper_1.ResponseHelper.success('Review retrieved successfully', reviewDto);
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
            ...
            return responseHelper_1.ResponseHelper.error('Failed to fetch review');
        }
    }
    async getUserReviews(userId) {
        ...
        try {
            ...
            if (!reviews || reviews.length === 0) {
                ...
                return responseHelper_1.ResponseHelper.success('No reviews found', {
                    reviews: [],
                    totalCount: 0,
                    currentPage: 1,
                    totalPages: 0,
            ...
            const reviewDtos = (0, reviewMapper_1.toReviewDtoList)(reviews);
            return responseHelper_1.ResponseHelper.success('Reviews retrieved successfully', {
                reviews: reviewDtos,
            ...
            return responseHelper_1.ResponseHelper.error('Failed to fetch reviews');
        }
    }
    async getTechnicianReviews(technicianId, page = 1, limit = 10) {
        const context = {
        ...
        try {
            ...
            const { reviews, totalCount } = await this._reviewRepository.findByTechnicianId(technicianId, page, limit);
            if (!reviews || reviews.length === 0) {
    ...
    async getTechnicianReviewStats(technicianId) {
        const context = {
        ...
        try {
            ...
            const stats = await this._reviewRepository.getTechnicianStats(technicianId);
            this._logger.info('Technician review stats retrieved successfully', {
                ...context,
                averageRating: stats.averageRating,
                totalReviews: stats.totalReviews,
            });
            return responseHelper_1.ResponseHelper.success('Review stats retrieved successfully', stats);
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
            ...
            return responseHelper_1.ResponseHelper.error('Failed to fetch review stats');
        }
    }
    async canUserReviewOrder(userId, orderId) {
        return await this._reviewRepository.canUserReviewOrder(userId, orderId);
    }
    async reportReview(userId, reviewId, reportData) {
        const context = {
        ...
        try {
            ...
            if (review.userId.toString() === userId) {
                ...
                return responseHelper_1.ResponseHelper.badRequest('You cannot report your own review');
            }
            // Validate reason
            if (!reportData.reason || reportData.reason.trim().length === 0) {
                this._logger.warn('Empty reason provided for report', context);
                return responseHelper_1.ResponseHelper.badRequest('Reason is required');
            }
            // Prepare report data
            ...
            // Save the report
            const result = await this._reviewRepository.reportReview(reviewId, reportDataForRepo);
            ...
            return responseHelper_1.ResponseHelper.success('Review reported successfully. Our team will review it shortly.', {
                reportId: result.reportId,
            });
        }
        catch (error) {
            ...
            if (errorMessage.includes('already reported')) {
                return responseHelper_1.ResponseHelper.badRequest('You have already reported this review');
            }
            return responseHelper_1.ResponseHelper.error('Failed to report review');
        }
    }
    // ==================== NOTIFICATION METHODS ====================
    async notifyTechnicianAboutNewReview(review, order) {
        try {
            const context = {
            ...
            this._logger.info('Sending new review notification to technician', context);
            // Get user details for personalized notification
            const UserModel = (await Promise.resolve().then(() => __importStar(require('../models/UserSchema')))).default;
            const user = await UserModel.findById(review.userId);
            const customerName = user?.fullName || 'A customer';
            await this._socketService.notifyReviewReceived(review.technicianId.toString(), review.rating, customerName);
            this._logger.info('New review notification sent to technician successfully', context);
        }
        catch (error) {
            // Don't fail the review creation if notification fails
            this._logger.error('Failed to send new review notification to technician', {
                reviewId: review._id.toString(),
                technicianId: review.technicianId.toString(),
    ...
    async notifyUserAboutReviewSubmission(userId, serviceName) {
        try {
            const context = {
            ...
            this._logger.info('Sending review submission confirmation to user', context);
            await this._socketService.sendLiveNotification(userId, {
                userId,
                userType: 'customer',
                type: 'system',
            ...
            this._logger.info('Review submission confirmation sent to user successfully', context);
        }
        catch (error) {
            this._logger.error('Failed to send review submission confirmation to user', {
                userId,
                error: error instanceof Error ? error.message : 'Unknown error',
            });
        }
    }
    async notifyTechnicianAboutReviewUpdate(review) {
        try {
            const context = {
            ...
            this._logger.info('Sending review update notification to technician', context);
            await this._socketService.sendLiveNotification(review.technciianId.toString(), {
            ...
            this._logger.info('Review update notification sent to technician successfully', context);
        }
        catch (error) {
            this._logger.error('Failed to send review update notification to technician', {
                reviewId: review._id.toString(),
                technicianId: review.technicianId.toString(),
                error: error instanceof Error ? error.message : 'Unknown error',
            });
    ...
    async notifyTechnicianAboutReviewDeletion(review) {
        try {
            const context = {
            ...
            this._logger.info('Sending review deletion notification to technician', context);
            await this._socketService.sendLiveNotification(review.technicianId.toString(), {
                userId: review.technicianId.toString(),
                userType: 'technician',
                type: 'system',
            ...
            this._logger.info('Review deletion notification sent to technician successfully', context);
        }
        catch (error) {
            this._logger.error('Failed to send review deletion notification to technician', {
                reviewId: review._id.toString(),
                technicianId: review.technicianId.toString(),
                error: error instanceof Error ? error.message : 'Unknown error',
            });
    ...
    async notifyUserAboutReportSubmission(userId) {
        try {
            const context = {
                operation: 'notifyUserAboutReportSubmission',
                userId,
            };
            this._logger.info('Sending report submission confirmation to user', context);
            await this._socketService.sendLiveNotification(userId, {
                userId,
                userType: 'customer',
                type: 'system',
                title: 'Report Submitted',
                message: 'Thank you for reporting this review. Our team will investigate and take appropriate action.',
                priority: 'medium',
                data: {
                    action: 'report_submitted',
                },
            });
            this._logger.info('Report submission confirmation sent to user successfully', context);
        }
        catch (error) {
            this._logger.error('Failed to send report submission confirmation to user', {
                userId,
                error: error instanceof Error ? error.message : 'Unknown error',
            });
        }
    }
    async notifyReviewAuthorAboutReport(review, reason) {
        try {
            const context = {
            ...
            this._logger.info('Report notification sent to review author successfully', context);
        }
        catch (error) {
    ...
    async notifyTechnicianAboutMilestone(technicianId, milestone) {
        try {
            const context = {
            ...
            this._logger.info('Milestone notification sent to technician successfully', context);
        }
        catch (error) {
