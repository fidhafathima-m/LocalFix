...
const sendPhoneOTP_1 = require("../utils/sendPhoneOTP");
const sendEmailOTP_1 = require("../utils/sendEmailOTP");
const responseHelper_1 = require("../utils/responseHelper");
const constants_1 = require("../constants");
class AuthService {
    constructor(userRepository, otpRepository, socialAccountRepository, logger) {
    ...
    async signup(signupData) {
        ...
        try {
            ...
            if (!email) {
                return responseHelper_1.ResponseHelper.badRequest('Email is required');
            }
            ...
            if (existingUser && existingUser.roles.includes(userType)) {
                ...
                return responseHelper_1.ResponseHelper.conflict('Email already registered for this role. Please use a different email or try logging in.');
            }
            ...
            const otpData = {
                otpHash,
                purpose: constants_1.OTPPurpose.SIGNUP,
                expiresAt: new Date(Date.now() + constants_1.OTP_CONFIG.EXPIRY_MS),
                email: email,
            };
            await this._otpRepository.create(otpData);
            this._logger.info('OTP record created successfully', {
                ...context,
                otpPurpose: constants_1.OTPPurpose.SIGNUP,
            });
            ...
            return responseHelper_1.ResponseHelper.success(`OTP sent to your email. Verify to complete signup.`);
        }
        catch (error) {
            ...
            return responseHelper_1.ResponseHelper.error(errorMessage);
        }
    }
    async sendEmailOTPAsync(email, otp) {
        try {
            // 🔥 TEMPORARY: Always log OTP for testing
            console.log(`🚨 OTP FOR TESTING - ${email}: ${otp}`);
            await (0, sendEmailOTP_1.sendEmailOTP)(email, otp);
    ...
    async verifyOtp(otpData) {
        ...
        try {
            ...
            if (!userType) {
                this._logger.warn('OTP verification attempted without user type', context);
                return responseHelper_1.ResponseHelper.badRequest('User type is required');
            }
            const record = await this._otpRepository.findLatest(phone, email, constants_1.OTPPurpose.SIGNUP);
            if (!record) {
                this._logger.warn('OTP record not found for verification', context);
                return responseHelper_1.ResponseHelper.badRequest(constants_1.AUTH_MESSAGES.OTP_NOT_FOUND);
            }
            if (record.expiresAt < new Date()) {
                ...
                return responseHelper_1.ResponseHelper.badRequest(constants_1.AUTH_MESSAGES.OTP_EXPIRED);
            }
            const isMatch = await bcrypt_1.default.compare(otp, record.otpHash);
            if (!isMatch) {
                this._logger.warn('Invalid OTP provided', {
                    ...context,
                    otpId: record._id,
                });
                return responseHelper_1.ResponseHelper.badRequest(constants_1.AUTH_MESSAGES.INVALID_OTP);
            }
            ...
            if (user) {
                ...
                if (!user.roles.includes(userType)) {
                    ...
                    const updateResult = await this._userRepository.update(user._id.toString(), {
                        roles: updatedRoles,
                    });
                    if (!updateResult) {
                        ...
                        return responseHelper_1.ResponseHelper.error('Failed to update user roles');
                    }
                    user = updateResult;
                    // Update password
                    if (password) {
                        const passwordHash = await bcrypt_1.default.hash(password, 10);
                        const passwordUpdateResult = await this._userRepository.update(user._id.toString(), {
                            passwordHash,
                        });
                    ...
                    if (fullName && user.fullName !== fullName) {
                        const nameUpdateResult = await this._userRepository.update(user._id.toString(), {
                            fullName,
                        });
                        if (nameUpdateResult) {
                    ...
                    this._logger.info('User already has the required role, updating profile if needed', {
                        ...context,
                        userId: user._id,
                    });
                ...
                if (!newUser) {
                    ...
                    return responseHelper_1.ResponseHelper.error('Failed to create user');
                }
                user = newUser;
            ...
            if (!user) {
                this._logger.error('User creation/update resulted in null user', context);
                return responseHelper_1.ResponseHelper.error('User creation/update failed');
            }
            const tokens = this.generateTokens(user);
            // Store refresh token in database
            await this._userRepository.storeRefreshToken(user._id.toString(), tokens.refreshToken);
            // Delete used OTP
            ...
            return responseHelper_1.ResponseHelper.success(constants_1.AUTH_MESSAGES.SIGNUP_SUCCESS, {
                user: userResponse,
    ...
    async refreshToken(refreshToken) {
        ...
        try {
            ...
            if (!refreshToken) {
                ...
                return responseHelper_1.ResponseHelper.unauthorized('Refresh token required');
            }
            ...
            if (decoded.type !== 'refresh') {
                ...
                return responseHelper_1.ResponseHelper.unauthorized('Invalid token type');
            }
            // Check if refresh token exists in database
            const user = await this._userRepository.findByRefreshToken(decoded._id, refreshToken);
            if (!user) {
                this._logger.warn('Refresh token not found in database', {
                    ...context,
                    userId: decoded._id,
                });
                return responseHelper_1.ResponseHelper.unauthorized('Invalid refresh token');
            }
            // Generate new tokens
            const tokens = this.generateTokens(user);
            ...
            await this._userRepository.updateRefreshToken(user._id.toString(), refreshToken, tokens.refreshToken);
            this._logger.info('Token refresh completed successfully', {
                ...context,
                userId: user._id,
            });
            return responseHelper_1.ResponseHelper.success('Token refreshed successfully', {
                accessToken: tokens.accessToken,
                refreshToken: tokens.refreshToken,
            });
        }
            ...
            if (error instanceof jsonwebtoken_1.default.TokenExpiredError) {
                ...
                return responseHelper_1.ResponseHelper.unauthorized('Refresh token expired');
            }
    ...
    async login(credentials) {
        ...
        try {
            ...
            if (role) {
                ...
                if (!user) {
                    this._logger.info('User not found with specific role, searching without role', {
                        ...context,
                        role: role,
                    });
                    ...
                    if (user && !user.roles.includes(role)) {
                        ...
                        return responseHelper_1.ResponseHelper.notFound(`${constants_1.AUTH_MESSAGES.USER_NOT_FOUND} for ${role} role`);
                    }
                }
            }
            else {
            ...
            if (!user) {
                ...
                return responseHelper_1.ResponseHelper.notFound(constants_1.AUTH_MESSAGES.USER_NOT_FOUND);
            }
            ...
            if (user.isDeleted) {
                ...
                return responseHelper_1.ResponseHelper.forbidden(constants_1.AUTH_MESSAGES.ACCOUNT_DELETED);
            }
            if (user.status === constants_1.UserStatus.BLOCKED) {
                ...
                return responseHelper_1.ResponseHelper.forbidden(constants_1.AUTH_MESSAGES.ACCOUNT_BLOCKED);
            }
            if (user.status !== constants_1.UserStatus.ACTIVE) {
                ...
                return responseHelper_1.ResponseHelper.forbidden(constants_1.AUTH_MESSAGES.ACCOUNT_INACTIVE);
            }
            ...
            if (!isMatch) {
                ...
                return responseHelper_1.ResponseHelper.notFound(constants_1.AUTH_MESSAGES.INVALID_CREDENTIALS);
            }
            ...
            return responseHelper_1.ResponseHelper.success(constants_1.AUTH_MESSAGES.LOGIN_SUCCESS, {
                user: userResponse,
                accessToken: tokens.accessToken,
                refreshToken: tokens.refreshToken,
            });
    ...
    async forgotPassword(phone, email, userType) {
        const context = {
        ...
        try {
            ...
            if (!phone && !email) {
                this._logger.warn('Forgot password attempted without email or phone', context);
                return responseHelper_1.ResponseHelper.badRequest(constants_1.AUTH_MESSAGES.EMAIL_OR_PHONE_REQUIRED);
            }
            ...
            if (userType) {
                ...
                switch (userType) {
                    case constants_1.UserRoles.SERVICE_PROVIDER:
                        expectedRole = constants_1.UserRoles.SERVICE_PROVIDER;
                        break;
                    case constants_1.UserRoles.ADMIN:
                        expectedRole = constants_1.UserRoles.ADMIN;
                        break;
                    case constants_1.UserRoles.USER:
                    default:
                        expectedRole = constants_1.UserRoles.USER;
                        break;
                }
                if (!user.roles.includes(expectedRole)) {
                    this._logger.warn('User does not have required role for forgot password', {
                        ...context,
                        userId: user._id,
                        userRoles: user.roles,
                        expectedRole: expectedRole,
                    });
                    return responseHelper_1.ResponseHelper.notFound(`${constants_1.AUTH_MESSAGES.USER_NOT_FOUND} for ${userType} role`);
                }
            }
            ...
            const otpData = {
                otpHash,
                purpose: constants_1.OTPPurpose.RESET,
                expiresAt: new Date(Date.now() + constants_1.OTP_CONFIG.EXPIRY_MS),
            };
            if (phone)
                otpData.phone = phone;
            ...
            const sentChannels = [];
            if (phone) {
                await (0, sendPhoneOTP_1.sendPhoneOTP)(phone, otp);
            ...
            return responseHelper_1.ResponseHelper.success(`OTP sent to ${sentChannels.join(', ')}.`);
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
    ...
    async resetPassword(resetData) {
        ...
        try {
            ...
            if (!password) {
                this._logger.warn('Password reset attempted without new password', context);
                return responseHelper_1.ResponseHelper.badRequest(constants_1.AUTH_MESSAGES.PASSWORD_REQUIRED);
            }
            let record;
            ...
            // Handle token-based reset
                ...
                try {
                    ...
                    if (decoded.purpose !== 'password_reset') {
                        ...
                        return responseHelper_1.ResponseHelper.badRequest(constants_1.AUTH_MESSAGES.INVALID_RESET_TOKEN);
                    }
                    ...
                    if (tokenIdentifier !== providedIdentifier) {
                        ...
                        return responseHelper_1.ResponseHelper.badRequest(constants_1.AUTH_MESSAGES.TOKEN_MISMATCH);
                    }
                    ...
                    if (decoded.userType !== userType) {
                        ...
                        return responseHelper_1.ResponseHelper.badRequest(constants_1.AUTH_MESSAGES.USER_TYPE_MISMATCH);
                    }
                    ...
                    return responseHelper_1.ResponseHelper.badRequest(constants_1.AUTH_MESSAGES.INVALID_OR_EXPIRED_TOKEN);
                }
            }
            else {
                this._logger.warn('Password reset attempted without OTP or token', context);
                return responseHelper_1.ResponseHelper.badRequest(constants_1.AUTH_MESSAGES.OTP_OR_TOKEN_REQUIRED);
            }
            const passwordHash = await bcrypt_1.default.hash(password, 10);
            ...
            await this._userRepository.updatePassword(identifier, passwordHash, userType);
            ...
            return responseHelper_1.ResponseHelper.success(constants_1.AUTH_MESSAGES.PASSWORD_RESET);
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
    ...
    async verifyResetOtp(otpData) {
        ...
        try {
            ...
            if (userType === constants_1.UserRoles.SERVICE_PROVIDER) {
                this._logger.debug('Verifying service provider user exists', {
                    ...context,
                    userType: constants_1.UserRoles.SERVICE_PROVIDER,
                });
                const identifier = phone || email;
                const user = await this._userRepository.findByIdentifier(identifier, constants_1.UserRoles.SERVICE_PROVIDER);
                if (!user) {
                    this._logger.warn('Service provider user not found for OTP verification', {
                        ...context,
                        identifier,
                        userType: constants_1.UserRoles.SERVICE_PROVIDER,
                    });
                    return responseHelper_1.ResponseHelper.notFound(`${userType} not found`);
                }
            ...
            this._logger.info('Reset OTP verification completed, temporary token generated', {
                ...context,
                tokenExpiresIn: '15m',
            });
            // Return the token in the expected structure
            return responseHelper_1.ResponseHelper.success(constants_1.AUTH_MESSAGES.OTP_VERIFIED, {
                token: tempToken,
    ...
    async resendOTP(phone, email, purpose, userType) {
        const context = {
            operation: 'resendOTP',
        ...
        try {
            ...
            if (!email && !phone) {
                this._logger.warn('OTP resend attempted without email or phone', context);
                return responseHelper_1.ResponseHelper.badRequest(constants_1.AUTH_MESSAGES.EMAIL_OR_PHONE_REQUIRED);
            }
            // Validate purpose parameter
            if (!purpose || !Object.values(constants_1.OTPPurpose).includes(purpose)) {
                this._logger.warn('Invalid OTP purpose provided', {
                    ...context,
                    providedPurpose: purpose,
                });
                return responseHelper_1.ResponseHelper.badRequest('Valid OTP purpose is required');
            }
            // For forgot password, check if user exists
            if (purpose === constants_1.OTPPurpose.RESET) {
                const identifier = phone || email;
            ...
            return responseHelper_1.ResponseHelper.success(`OTP resent to ${sentChannels.join(', ')}`);
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
    ...
    async facebookLogin(accessToken, userID) {
        ...
        try {
            ...
            const fbRes = await axios_1.default.get(`https://graph.facebook.com/${userID}?fields=id,name,email,picture&access_token=${accessToken}`);
            ...
            let user;
            if (!account) {
                this._logger.info('No existing Facebook account found, creating new association', {
                    ...context,
                    facebookId: id,
                });
            ...
            return responseHelper_1.ResponseHelper.success(constants_1.AUTH_MESSAGES.FACEBOOK_LOGIN_SUCCESS, {
                accessToken: tokens.accessToken,
                refreshToken: tokens.refreshToken,
                user: userResponse,
            });
    ...
    async googleAuth(socialData) {
        ...
        try {
            ...
            let socialAccount = await this._socialAccountRepository.findByProviderId(googleId);
            let user;
            if (socialAccount) {
                ...
                user = await this._userRepository.findById(socialAccount.userId.toString());
            }
            else {
                this._logger.info('No existing Google account found, creating new association', {
                    ...context,
                    googleId: googleId,
                });
                ...
                if (!user) {
                    ...
                    user = await this._userRepository.create({
                        ...
                        email: email,
                        isVerified: true,
                        roles: userType === constants_1.UserRoles.SERVICE_PROVIDER
                            ? [constants_1.UserRoles.SERVICE_PROVIDER]
                            : ['user'],
                        applicationStatus: 'not-applied',
                    });
                    ...
                    if (userType === constants_1.UserRoles.SERVICE_PROVIDER &&
                        !user.roles.includes(constants_1.UserRoles.SERVICE_PROVIDER)) {
                        const updatedRoles = [...user.roles, constants_1.UserRoles.SERVICE_PROVIDER];
                ...
                socialAccount =
                    await this._socialAccountRepository.findByUserIdAndProvider(user._id, 'google');
                if (!socialAccount) {
            ...
            return responseHelper_1.ResponseHelper.success(constants_1.AUTH_MESSAGES.GOOGLE_AUTH_SUCCESS, {
                user: userResponse,
                accessToken: tokens.accessToken,
                refreshToken: tokens.refreshToken,
            });
    ...
    async logout(userId, refreshToken) {
        ...
        try {
            ...
            return responseHelper_1.ResponseHelper.success('Logged out successfully');
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
