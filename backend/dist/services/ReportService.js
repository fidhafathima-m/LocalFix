...
class ReportService {
            ...
            switch (request.reportType) {
                case 'financial':
                    if (!request.startDate || !request.endDate) {
                        throw new Error('Start date and end date are required for financial reports');
                    }
                    reportData = await this.generateFinancialReport(request.startDate, request.endDate);
                    break;
                case 'customer':
                    if (!request.startDate || !request.endDate) {
                        throw new Error('Start date and end date are required for customer reports');
                    }
                    reportData = await this.generateCustomerReport(request.startDate, request.endDate);
                    break;
                case 'technician':
                    if (!request.startDate || !request.endDate) {
                        throw new Error('Start date and end date are required for technician reports');
                    }
                    reportData = await this.generateTechnicianReport(request.startDate, request.endDate);
                    break;
                default:
                    reportData = await this._reportRepository.getDashboardReportData(request.startDate, request.endDate);
            }
            const result = await this._reportRepository.exportReport(reportData, request.format);
    ...
    async generateFinancialReport(startDate, endDate) {
        ...
        try {
            ...
            const financialData = await this._reportRepository.getFinancialReportData(startDate, endDate);
    ...
    async generateCustomerReport(startDate, endDate) {
        ...
        try {
            ...
            const customerData = await this._reportRepository.getCustomerReportData(startDate, endDate);
    ...
    async generateTechnicianReport(startDate, endDate) {
        ...
        try {
            ...
            const technicianData = await this._reportRepository.getTechnicianReportData(startDate, endDate);
