...
class PaymentManagementService {
    constructor(paymentRepository, logger, walletService, socketService) {
        this._paymentRepository = paymentRepository;
        this._logger = logger;
        this._walletService = walletService;
        this._socketService = socketService;
    }
    async getPayments(page = 1, limit = 10, search, status, startDate, endDate) {
        const context = {
        ...
        try {
            ...
            if (search) {
                ...
                const allSearchResults = await this._paymentRepository.search(search, 10000, filter);
            ...
            return (0, paymentMapper_1.toPaymentListResponseDto)(payments, total, page, limit);
        }
        catch (error) {
    ...
    async getPaymentById(paymentId) {
        ...
        try {
            ...
            if (!mongoose_1.Types.ObjectId.isValid(paymentId)) {
                ...
                throw new Error(constants_1.PAYMENT_MESSAGES.INVALID_PAYMENT_ID);
            }
            const payment = await this._paymentRepository.findById(paymentId);
            if (!payment) {
                this._logger.warn('Payment not found', context);
                throw new Error(constants_1.PAYMENT_MESSAGES.PAYMENT_NOT_FOUND);
            }
            ...
            return (0, paymentMapper_1.toPaymentResponseDto)(payment);
        }
        catch (error) {
            if (error instanceof Error) {
    ...
    async getPaymentStats() {
        ...
        try {
            ...
            return (0, paymentMapper_1.toPaymentStatsDto)(stats);
        }
        catch (error) {
    ...
    async processRefund(paymentId, refundData) {
        ...
        try {
            ...
            if (payment.status !== 'success') {
                ...
                throw new Error(constants_1.PAYMENT_MESSAGES.REFUND_NOT_ALLOWED);
            }
            ...
            const refundResponse = await this._walletService.refundToWallet(userId, refundBookingId, payment.amount, refundData?.reason || 'Admin initiated refund');
            ...
            if (!updatedPayment) {
                ...
                throw new Error(constants_1.PAYMENT_MESSAGES.FAILED_PROCESS_REFUND);
            }
            ...
            await this.notifyUserAboutRefund(userId, payment.amount, payment.providerOrderId, refundData?.reason, refundResponse.data?.newBalance);
    ...
    async notifyUserAboutRefund(userId, amount, orderId, reason, newBalance) {
        const context = {
        ...
        try {
            ...
            if (this._socketService) {
                await this._socketService.sendLiveNotification(userId, notificationData);
            }
    ...
    convertToCSV(payments) {
        ...
        const rows = payments.map(payment => {
            const paymentDto = (0, paymentMapper_1.toPaymentResponseDto)(payment);
            return [
