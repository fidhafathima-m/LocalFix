...
class ReportRepository {
    async getDashboardReportData(startDate, endDate) {
        const dateFilter = {};
        ...
        const [totalRevenue, totalBookings, totalUsers, totalTechnicians, revenueTrend, topTechnicians, customerSatisfaction, paymentMethods,] = await Promise.all([
    ...
    async getFinancialReportData(startDate, endDate) {
        ...
        const financialData = await OrderSchema_1.default.aggregate([
            {
                $match: {
                    ...dateFilter,
                    'payment.status': 'paid',
        ...
        return {
            ...
            summary: {
                ...
                totalBookings: financialData.reduce((sum, item) => sum + item.bookings, 0),
                period: `${startDate.toDateString()} to ${endDate.toDateString()}`,
            },
        };
    }
    async getCustomerReportData(startDate, endDate) {
        const dateFilter = {
            createdAt: {
                $gte: startDate,
                $lte: endDate,
        ...
        const customerData = await UserSchema_1.default.aggregate([
            {
                $match: {
                    ...dateFilter,
                    roles: 'user',
    ...
    async getTechnicianReportData(startDate, endDate) {
        const dateFilter = {
            createdAt: {
                $gte: startDate,
                $lte: endDate,
        ...
        const technicianData = await TechnicianSchema_1.Technician.aggregate([
            {
                $match: {
                    status: 'approved',
                },
    ...
    addPDFContent(doc, data) {
        ...
        if (data.dateRange) {
            doc
                .fontSize(12)
                .text(`Period: ${data.dateRange.start ? new Date(data.dateRange.start).toLocaleDateString() : 'N/A'} - ${data.dateRange.end ? new Date(data.dateRange.end).toLocaleDateString() : 'N/A'}`, { align: 'center' });
        }
        doc.moveDown();
        // Overview Section
        if (data.overview) {
            ...
            doc
                ...
                .text(`Total Bookings: ${overview.totalBookings?.toLocaleString() || 0}`)
                .text(`Total Users: ${overview.totalUsers?.toLocaleString() || 0}`)
                .text(`Total Technicians: ${overview.totalTechnicians?.toLocaleString() || 0}`);
            doc.moveDown();
        }
        // Revenue Trend
        if (data.revenueTrend && data.revenueTrend.length > 0) {
            ...
            data.revenueTrend.forEach((item) => {
                doc
                    .fontSize(10)
                    .text(`${item.period}: ₹${item.revenue?.toLocaleString() || 0} (Profit: ₹${item.profit?.toLocaleString() || 0})`);
            });
            doc.moveDown();
        }
        // Top Technicians
        if (data.topTechnicians && data.topTechnicians.length > 0) {
            ...
            doc.moveDown(0.5);
            data.topTechnicians.forEach((tech, index) => {
                doc
                    .fontSize(10)
                    .text(`${index + 1}. ${tech.name || 'Unknown'}`)
                    .text(`   Rating: ${tech.rating?.toFixed(1) || 'N/A'} | Jobs: ${tech.jobs || 0} | Revenue: ₹${tech.revenue?.toLocaleString() || 0}`);
            });
            doc.moveDown();
        }
        ...
        if (data.customerSatisfaction && data.customerSatisfaction.length > 0) {
            ...
            doc.moveDown(0.5);
            data.customerSatisfaction.forEach((item) => {
                doc
                    .fontSize(10)
                    .text(`${item.stars} Stars: ${item.count} reviews (${item.percentage?.toFixed(1)}%)`);
            });
            doc.moveDown();
        }
        // Payment Methods
        if (data.paymentMethods && data.paymentMethods.length > 0) {
            ...
            doc.moveDown(0.5);
            const totalAmount = data.paymentMethods.reduce((sum, method) => sum + method.amount, 0);
            data.paymentMethods.forEach((method) => {
                const percentage = totalAmount > 0 ? (method.amount / totalAmount) * 100 : 0;
                doc
                    .fontSize(10)
                    .text(`${method.method.toUpperCase()}: ₹${method.amount?.toLocaleString() || 0} (${percentage.toFixed(1)}%)`);
            });
        }
        // Footer
        doc.moveDown(2);
    ...
    async generateExcel(data) {
        try {
            ...
            if (data.customerSatisfaction && data.customerSatisfaction.length > 0) {
                const satisfactionSheet = workbook.addWorksheet('Customer Satisfaction');
    ...
    async getTotalTechnicians(dateFilter) {
    ...
    async getTopTechnicians(dateFilter) {
    ...
    async getCustomerSatisfaction(dateFilter) {
    ...
    async getPaymentMethods(dateFilter) {
    ...
    async getMonthlyRevenue(date, additionalFilter) {
        const monthFilter = {
            createdAt: {
