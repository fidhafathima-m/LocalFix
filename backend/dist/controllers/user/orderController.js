    ...
    constructor(orderService, logger) {
        this.getUserOrders = async (req, res) => {
            ...
            try {
                ...
                if (!userId) {
                    this._logger.warn('Get user orders failed - authentication required', context);
        ...
        this.cancelOrder = async (req, res) => {
            ...
            try {
                ...
                if (!userId) {
                    this._logger.warn('Cancel order failed - authentication required', context);
                    const errorResponse = responseHelper_1.ResponseHelper.unauthorized('Authentication required');
                    res.status(errorResponse.statusCode).json(errorResponse);
                    return;
                }
                ...
                const result = await this._orderService.cancelOrder(userId, orderId, reason);
        ...
        this.rescheduleOrder = async (req, res) => {
            const userId = req.user?.id;
            const { orderId } = req.params;
            ...
            try {
                ...
                if (!userId) {
                    this._logger.warn('Reschedule order failed - authentication required', context);
                    const errorResponse = responseHelper_1.ResponseHelper.unauthorized('Authentication required');
                    res.status(errorResponse.statusCode).json(errorResponse);
                    return;
                }
                if (!newDate || !newTimeSlot) {
                    ...
                    const badRequestResponse = responseHelper_1.ResponseHelper.badRequest('New date and time slot are required');
                    res.status(badRequestResponse.statusCode).json(badRequestResponse);
                    return;
                }
                ...
                if (isNaN(scheduledAt.getTime())) {
                    this._logger.warn('Reschedule order failed - invalid date format', context);
                    const badRequestResponse = responseHelper_1.ResponseHelper.badRequest('Invalid date format');
                    res.status(badRequestResponse.statusCode).json(badRequestResponse);
                    return;
                }
                // Check if the new date is in the future
                if (scheduledAt <= new Date()) {
                    this._logger.warn('Reschedule order failed - date must be in future', context);
                    const badRequestResponse = responseHelper_1.ResponseHelper.badRequest('New date must be in the future');
                    res.status(badRequestResponse.statusCode).json(badRequestResponse);
                    return;
                }
                const result = await this._orderService.rescheduleOrder(userId, orderId, newDate, newTimeSlot);
                if (!result.success) {
        ...
        this.createOrderFromBooking = async (req, res) => {
            const userId = req.user?.id;
            ...
            try {
                ...
                if (!userId) {
                    this._logger.warn('Create order failed - authentication required', context);
                    const errorResponse = responseHelper_1.ResponseHelper.unauthorized('Authentication required');
                    res.status(errorResponse.statusCode).json(errorResponse);
                    return;
                }
                if (!bookingId || !paymentData) {
                    this._logger.warn('Create order failed - missing required fields', context);
                    const badRequestResponse = responseHelper_1.ResponseHelper.badRequest('Booking ID and payment data are required');
                    res.status(badRequestResponse.statusCode).json(badRequestResponse);
                    return;
                }
                const result = await this._orderService.createOrderFromBooking(bookingId, paymentData);
        ...
        this.getOrderByBookingId = async (req, res) => {
            const userId = req.user?.id;
            ...
            try {
                ...
                const result = await this._orderService.getOrderByBookingId(userId, bookingId);
        ...
        this.updateOrderPayment = async (req, res) => {
            const userId = req.user?.id;
            const { orderId } = req.params;
            ...
            try {
                ...
                if (!userId) {
                    this._logger.warn('Update payment failed - authentication required', context);
                    const errorResponse = responseHelper_1.ResponseHelper.unauthorized('Authentication required');
                    res.status(errorResponse.statusCode).json(errorResponse);
                    return;
                }
                const result = await this._orderService.updateOrderPayment(orderId, paymentData);
