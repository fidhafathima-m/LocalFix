...
export const userProfileApi = {
  updateUserProfile: (updateData: any) => {
    ...
    return api.put<ApiResponse<any>>(
      ADMIN_ROUTES.UPDATE_USER_PROFILE,
      transformedData,
    );
  },
  ...
  createAddress: (addressData: AddressFormData) =>
    api.post<ApiResponse<{ address: Address }>>(
      ADMIN_ROUTES.CREATE_ADDRESS,
      addressData,
    ),

  updateAddress: (addressId: string, addressData: Partial<AddressFormData>) =>
    api.put<ApiResponse<{ address: Address }>>(
      ADMIN_ROUTES.UPDATE_ADDRESS(addressId),
      addressData,
    ),
  ...
  setDefaultAddress: (addressId: string) =>
    api.patch<ApiResponse<{ address: Address }>>(
      ADMIN_ROUTES.SET_DEFAULT_ADDRESS(addressId),
    ),
  checkTechnicianAvailability: (data: {
    technicianId: string;
    scheduledAt: string;
    timeSlot: string;
  }) =>
    api.post<ApiResponse<{ available: boolean; message?: string }>>(
      ADMIN_ROUTES.CHECK_TECHNICIAN_AVAILABILITY,
      data,
    ),
};
