...
export const trackingService = {
  async getTrackingDetails(
    ...
    const response = await api.get<ApiResponse<ServiceTracking>>(
  ...
  async getTechnicianLocation(bookingId: string): Promise<
    ApiResponse<{
      latitude: number;
      longitude: number;
      lastUpdated: string;
  ...
  > {
    ...
    const response = await api.get<ApiResponse<any>>(
      `/bookings/${bookingId}/technician-location`
    );
    return response.data;
  },
};
